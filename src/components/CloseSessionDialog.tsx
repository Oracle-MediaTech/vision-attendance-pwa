...
const cellKey = (svc: number, cat: IncomeCategory, m: PaymentMethod) =>
   `${svc}:${cat}:${m}`;
...
function useSeedCells(open: boolean, services: ISessionService[]) {
   if (!open) return {};
   const seeded: Record<string, string> = {};
   for (const svc of services) {
      for (const inc of svc.incomes ?? []) {
         seeded[cellKey(svc.order, inc.category, inc.method)] = String(
            inc.amount,
         );
      }
   }
...
export default function CloseSessionDialog({
   ...
   const seededCells = useSeedCells(open, services);
   const [cells, setCells] = useState<Record<string, string>>(seededCells);
