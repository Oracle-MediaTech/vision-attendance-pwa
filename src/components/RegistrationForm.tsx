...
const MATRIC_MSG = "Format: UG/PG/DE/YY/NNN (e.g. UG/20/1234)";
const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

type MemberTab = "non-worker" | "worker" | "children";

const REQUIRED_FIELDS = new Set(["firstName", "lastName", "address", "gender"]);
...
function validateField(name: string, value: string, tab: MemberTab): string {
   ...
   if (name === "churchStatus" && tab === "non-worker" && !trimmed)
      return "Required";
   if (name === "email" && trimmed && !EMAIL_REGEX.test(trimmed))
   ...
   useImperativeHandle(
      ...
      () => ({
         reset() {
            ...
            setClientErrors({});
            setFormValues({});
            formElRef.current?.scrollIntoView({
               behavior: "smooth",
               block: "start",
            });
         },
   ...
   return (
      <div className="p-4">
         ...
         >
            ...
            {/* Church status: only shown on Non-Worker and children tab. Worker tab auto-sets MEMBER. */}
            {tab !== "worker" && (
               <div>
                  <select
                     name="churchStatus"
